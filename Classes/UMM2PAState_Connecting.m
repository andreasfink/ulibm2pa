...
@implementation UMM2PAState_Connecting
...
- (UMM2PAState *)initWithLink:(UMLayerM2PA *)link status:(M2PA_Status)statusCode
{
    ...
    {
        if((_statusCode == M2PA_STATUS_DISCONNECTED) && (oldStatus != M2PA_STATUS_DISCONNECTED) && (_link.sctpLink.status != UMSOCKET_STATUS_OFF))
        {
            [self eventPowerOff];
        }
    }
